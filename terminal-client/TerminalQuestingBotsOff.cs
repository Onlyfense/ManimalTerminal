...
namespace Manimal.Terminal
{
    ...
    internal static class TerminalQuestingBotsOff
    {
        internal static void TryPatch(Harmony h)
        {
            ...
            foreach (var t in new[] { pmc, pscav })
            {
                ...
                if (canSpawn != null)
                {
                    h.Patch(canSpawn, prefix: new HarmonyMethod(typeof(TerminalQuestingBotsOff), nameof(SkipFalse)));
                    gated++;
                }
                var allowed = AccessTools.DeclaredMethod(t, "GetNumberOfBotsAllowedToSpawn");
                if (allowed != null)
                {
                    h.Patch(allowed, prefix: new HarmonyMethod(typeof(TerminalQuestingBotsOff), nameof(SkipZero)));
                    gated++;
                }
            }
            // the wave killer (2026-08-17 log: 'Suppressing boss wave ... or too many
            // bosses' x31): terminal's ENTIRE population ships as BossLocationSpawn
            ...
            if (suppress != null)
            {
                h.Patch(suppress, prefix: new HarmonyMethod(typeof(TerminalQuestingBotsOff), nameof(SkipFalse)));
                gated++;
            }
            ...
            if (delayGetter != null)
            {
                h.Patch(delayGetter, prefix: new HarmonyMethod(typeof(TerminalQuestingBotsOff), nameof(SkipFalse)));
                gated++;
            }

            if (gated > 0)
                Plugin.Log.LogInfo($"[QuestingBotsOff] detected — {gated} method(s) gated (spawn generators + boss suppression + start delay); QB muted on terminal ONLY");
        }
