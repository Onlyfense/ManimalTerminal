...
{
    // retail lens flares, 1:1 port of IcebreakerFlares: 3011 MultiFlare.FlareLight
    // components (per-lamp flare sprites) rebuilt from the extraction sidecar with
    // the GAME's own MultiFlare classes — the scheduler already exists
    // (AbstractApplication.CreateTechnicalSystems), so we only recreate what the
    // scene carried: one FlareSceneSettings (atlas + batch materials, Awake ->
    // SetupRenderer) and the FlareLights (OnEnable self-registers). data is
    // all-primitive — no asset refs beyond the atlas texture, which ships as a PNG.
    internal static class TerminalFlares
    {
        private static bool _built;

        private static string Dir =>
            SysIoPath.Combine(SysIoPath.GetDirectoryName(typeof(Plugin).Assembly.Location) ?? ".", "plugin-data", "flares");
        ...
        internal static void TryBuild()
        {
            ...
            // retail's tuned property sets ride the sidecar — critically _DepthOffset
            // (on-surface depth tolerance: flares SIT ON flat lamp surfaces, and a
            // zero-default material loses the depth test to its own lamp — the
            // vanishing-flare bug)
            ...
            {
                _atlas = atlas,
                _normalBatch = Batch(s?["_normalBatch"], normalMat, 16384),
                _shitBatch = Batch(s?["_shitBatch"], shitMat ?? normalMat, 512),
                _flareOverlapSettings = overlap,
            };

            // ---- FlareSceneSettings: fields must land BEFORE Awake (inactive-GO trick) ----
            var host = new GameObject("Terminal_FlareSettings");
            ...
            host.SetActive(true); // Awake -> SetupRenderer(settings)
        ...
        // convention as the extraction side) — one walk, then O(1) lookups for 3011 rows
